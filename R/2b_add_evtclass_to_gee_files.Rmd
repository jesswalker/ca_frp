...
# Get relevant filenames
  filenames_in <- list.files(file.path(path_in, "data"), pattern = "*_plus_fire_info_slim_setyear.csv")
...
joined_file <- files_w_fire_info %>% left_join(select(files_w_frp_info, MaxFRP, index, sample, satellite, pixelarea, starts_with("class")), by = c("index", "MaxFRP"))
