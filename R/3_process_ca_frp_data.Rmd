...
_Input files_:  mxda1_gee_fire_info_yyyy.csv, where yyyy = 2002-2020  
...
path.plots <- file.path(path_in, "output/plots")
...
# Get the relevant filename
  #filenames.in <- list.files(file.path(path_in, "data"), pattern = "*_plus_fire_info.csv")

  files_all <- read.csv(file.path(path_data, "maxFRP_CA_gte4sqkm_2014to2020_fireInfo_evtClasses.csv"), header = T)

# Read all files to a single file. Suppress warnings b/c of spurious "incomplete last line" error
  #suppressWarnings(files.all <- do.call(rbind, lapply(file.path(path_in, "data/intersected", filenames.in), read.csv, row.names=NULL)))
```
