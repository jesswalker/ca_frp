...
_Output file_:  ca_frp_area_data.RData
...
x_noclasses_ptid_gt20000 <-  subset(x_noclasses, ptid2 > 20000)
...
lst_fireslte10000 <- x_noclasses_ptid_lte10000 %>% 
            group_by(ptid) %>%
            do(create_new_fire_df(.))

lst_fireslte20000 <- x_noclasses_ptid_lte20000 %>% 
            group_by(ptid) %>%
            do(create_new_fire_df(.))

lst_firesgt20000 <- x_noclasses_ptid_gt20000 %>% 
            group_by(ptid) %>%
            do(create_new_fire_df(.))

lst_fires <- rbind(lst_fireslte10000, lst_fireslte20000, lst_firesgt20000)
...
# If it's been successful until now, save the image 

save.image(file = file.path(path_in, "R",'processed_data_after_creating_new_df.RData'))
