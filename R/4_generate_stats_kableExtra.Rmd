...
data_out <- "data_from_script4_k.Rdata"
...
# Count the number of times each point appears: 1 = 1st fire, 2 = 2nd, etc.
  get_fire_order = function(df) {
    ...
    df$reburn <- 0 
    df %>%
       mutate(reburn = case_when(burn_num > 1 ~ 1)) # reburn: no = 0, yes = 1
...
RData file is saved to `r file.path(path_data, data_out)`
```{r, echo=FALSE}
