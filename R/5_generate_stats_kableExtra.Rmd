...
####_5_generate_stats_and_plots.Rmd_

This script generates stats and plots based on previously processed data.  

_Input files_:  new_fire_df.RData
...
rdata_out <- "script5_stats_output.Rdata"
#source(file.path(path_r, "ca_functions.R"))
```

```{r, echo=FALSE, message=FALSE}

load(file.path(path_r,'test_of_slim_code4_output.Rdata')) #'new_fire_df.RData'
...
```
The ingested dataset has redundancies because FRP pixels are paired with each coincident fire perimeter from all previous years. After processing, an example of a point with multiple associated fires (without class info) is:
...
Fire year is assumed to be MODIS year for all fires 2014 - 2020. Other notes:
...
# ---------------------------------------------------------------- #
  ...
  x_fires_evt <- merge(x_fires_evt, evt_classes, by.x = "max_evt_cat", by.y = "evt_number")
  ...
  # Rearrange 
  x_fires_evt <- x_fires_evt %>% select(ptid:evt_group, max_evt_cat)
...
x_fires_evt_1stfrp <- rbind(x.temp1, x.temp2)
...
RData file is saved to `r file.path(path_r, rdata_out)`
```{r, echo=FALSE}

# Save data and environment settings   
print(paste0("R data file saved to ", file.path(path_r, rdata_out)))
save.image(file = file.path(path_r, rdata_out))
