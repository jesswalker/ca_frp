...
# Input: maxFRP_CA_gte4sqkm_yyyy_plus_fire_info_setyear.csv
...
file_out <- paste0("maxFRP_CA_gte4sqkm_", startyr, "to", endyr, "_fireInfo_evtClasses.csv")
...
# ---------------------------------------------------------------- #
#### Merge files that have historical fire info but lack EVT classes ####
# ---------------------------------------------------------------- #
...
joined_file <- files_w_fire_info %>% left_join(select(files_w_evt_info, index, MaxFRP, sample, satellite, pixelarea, starts_with("class")), by = c("index", "pixelarea"))

# Why not and the other satellite metrics? B/c they are set to NA for all years in which the MODIS year != fire year, so it will not be equal between files. 
...
write.csv(joined_file, file.path(path_data, file_out), row.names = FALSE)
